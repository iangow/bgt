...
fk_sql <- "CASE WHEN sent_count > 0 AND word_count > 0
              THEN .39 * (word_count /  sent_count) + 11.8 * (num_syllables / word_count) - 15.59
              END"
...
ari_sql <- "CASE WHEN word_count > 0 AND sent_count > 0
            THEN 4.71 * (nchars / word_count) +
                    0.5 * (word_count / sent_count) - 21.43 END"

system.time({
    processed <-
        syllable_data %>%
        inner_join(nchars) %>%
        mutate(monosyl_count=sql("(syllable_data->'syllable_counts'->'1')::text::float8"),
               bisyl_count=sql("(syllable_data->'syllable_counts'->'2')::text::float8")) %>%
        ...
        mutate(multisyl_count = word_count - monosyl_count) %>%
...
syllable_data %>%
    mutate(monosyllables=sql("(syllable_data->'syllable_counts'->'1')::text::int")) %>%
    select(-syllable_data)
