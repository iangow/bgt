---
title: "Calculating ELF (\"Easy Listening Factor\")"
author: "Ian Gow"
date: "May 27, 2015"
output: pdf_document
---
...
```{r get_data_function, cache=TRUE}
    ...
    sql <- paste0("
        ...
        sentences AS (
            SELECT file_name, category, 
                elf(sent_tokenize(all_text)) AS elf,
                fog_alt(all_text) AS fog
            FROM call_text)
        
        SELECT file_name, category, 
            array_sum(elf) AS elf_sum, 
            array_avg(elf) AS elf_mean,
            fog
        FROM sentences")
...
```{r get_call_list, include=FALSE, cache=TRUE}
library(RPostgreSQL)
pg <- dbConnect(PostgreSQL())
...
num_calls <- as.integer(dbGetQuery(pg, "SELECT COUNT(*) FROM streetevents.calls WHERE call_type=1")[,1])
...
Thus to get data for all `r prettyNum(num_calls, big.mark = ",")` earnings conference calls 
would require about `r prettyNum(time_taken[["elapsed"]]/3600 * num_calls/n, digits=2)` hours.

```{r, echo=FALSE, cache=TRUE, dependson="get_sample", include=FALSE}
corr <- cor(elf_data[, c("elf_sum", "elf_mean", "fog")])
```

The correlation between mean ELF and Fog is **`r prettyNum(corr["elf_mean", "fog"], digits=4)`**.

\clearpage
Here is the Python function `elf` used inside PostgreSQL to produce the ELF data.
```{r, echo=FALSE, results="markup", comment=NA}
temp <- paste(system(" psql -h iangow.me -d crsp -c '\\sf elf(text[])'", intern=TRUE), collapse="\n")
cat(temp)
```
